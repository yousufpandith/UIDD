...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn, CWallet* pwallet, bool fProofOfStake)
{
	...
	pblock->vtx.push_back(CTransaction());  // UIDD: Add dummy tx, will be the coinbase tx on PoW. or coinstake tx on PoS.
...

    {
        ...
        while (!vecPriority.empty()) {
            ...
            // double check that there are no double spent zUIDD spends in this block or tx
            if (tx.IsZerocoinSpend()) {
                int nHeightTx = 0;
                ...
                //This zUIDD serial has already been included in the block, do not add this tx.
                if (fDoubleSerial)
                    continue;
            }
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("UiddMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("UiddMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("UIDDMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("UIDD-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake)
	{
        ...
        LogPrintf("Running UiddMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
