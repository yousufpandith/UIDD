...
ZerocoinTutorial()
{
	...
	try {
        ...
        libzerocoin::PrivateCoin newCoin(params, libzerocoin::CoinDenomination::ZQ_FIVECENTS);
        ...
        libzerocoin::Accumulator accumulator(params,libzerocoin::CoinDenomination::ZQ_FIVECENTS);

		// Add several coins to it (we'll generate them here on the fly).
		for (uint32_t i = 0; i < COINS_TO_ACCUMULATE; i++) {
            libzerocoin::PrivateCoin testCoin(params, libzerocoin::CoinDenomination::ZQ_FIVECENTS);
