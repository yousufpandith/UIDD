...
Testb_Accumulator()
{
	...
	try {
		// Accumulate the coin list from first to last into one accumulator
            Accumulator accOne(&gg_Params->accumulatorParams,libzerocoin::CoinDenomination::ZQ_FIVECENTS);
            Accumulator accTwo(&gg_Params->accumulatorParams,libzerocoin::CoinDenomination::ZQ_FIVECENTS);
            Accumulator accThree(&gg_Params->accumulatorParams,libzerocoin::CoinDenomination::ZQ_FIVECENTS);
            Accumulator accFour(&gg_Params->accumulatorParams,libzerocoin::CoinDenomination::ZQ_FIVECENTS);
...
Testb_MintCoin()
{
	try {
		...
		timer.start();
		for (uint32_t i = 0; i < TESTS_COINS_TO_ACCUMULATE; i++) {
            ggCoins[i] = new PrivateCoin(gg_Params,CoinDenomination::ZQ_FIVECENTS);
		}
		timer.stop();
	} catch (exception &e) {
		return false;
...
Testb_MintAndSpend()
{
	try {
        ...
        Accumulator acc(&gg_Params->accumulatorParams,CoinDenomination::ZQ_FIVECENTS);
