...
Test_Accumulator()
{
	...
	try {
		// Accumulate the coin list from first to last into one accumulator
            Accumulator accOne(&g_Params->accumulatorParams, CoinDenomination::ZQ_FIVECENTS);
            Accumulator accTwo(&g_Params->accumulatorParams,CoinDenomination::ZQ_FIVECENTS);
            Accumulator accThree(&g_Params->accumulatorParams,CoinDenomination::ZQ_FIVECENTS);
            Accumulator accFour(&g_Params->accumulatorParams,CoinDenomination::ZQ_FIVECENTS);
...
Test_MintCoin()
{
	...
	try {
		...
		for (uint32_t i = 0; i < TESTS_COINS_TO_ACCUMULATE; i++) {
            gCoins[i] = new PrivateCoin(g_Params,libzerocoin::CoinDenomination::ZQ_FIVECENTS);
...
bool Test_InvalidCoin()
{
	...
	try {
		...
		PublicCoin pubCoin2(g_Params, coinValue, ZQ_FIVECENTS);
...
Test_MintAndSpend()
{
	try {
        ...
        Accumulator acc(&g_Params->accumulatorParams,CoinDenomination::ZQ_FIVECENTS);
