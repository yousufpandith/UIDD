	...
	if (nHeight > 0)
	{
		...
		else if (nHeight <= Params().LAST_POW_BLOCK())
		{
			...
			else
			{
				...
				nSubsidy = 98841699 * CENT; // Satoshis. (1684806479500000 (Number of satoshis in circulation on BTC block 507850) - (101 * COIN)) / (Params().LAST_POW_BLOCK() - 202);
	...
	if(block.IsProofOfStake())
	{
		...
		for (const CTransaction& tx : block.vtx)
		{
			for (const CTxIn& in : tx.vin)
			{
				...
				for (const CTxIn& UIDDIn : StakingInputs) {
					if (UIDDIn.prevout == in.prevout) {
