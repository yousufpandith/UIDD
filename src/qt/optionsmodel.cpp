...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::UIDD);
    ...
    if (!settings.contains("nAnonymizeUiddAmount"))
        settings.setValue("nAnonymizeUiddAmount", 1000);

    nAnonymizeUiddAmount = settings.value("nAnonymizeUiddAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeUiddAmount"))
        SoftSetArg("-anonymizeuiddamount", settings.value("nAnonymizeUiddAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in uidd.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeUiddAmount:
            return QVariant(nAnonymizeUiddAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeUiddAmount:
            nAnonymizeUiddAmount = value.toInt();
            settings.setValue("nAnonymizeUiddAmount", nAnonymizeUiddAmount);
            emit anonymizeUiddAmountChanged(nAnonymizeUiddAmount);
            break;
        case CoinControlFeatures:
