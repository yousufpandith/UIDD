...
#include "ui_zuiddcontroldialog.h"
...
std::list<std::string> ZUIDDControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZUIDDControlDialog::listMints;

ZUIDDControlDialog::ZUIDDControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZUIDDControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZUIDDControlDialog::~ZUIDDControlDialog()
{
    delete ui;
}

void ZUIDDControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZUIDDControlDialog::updateList()
{
...
void ZUIDDControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZUIDDControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZUIDD_int->setText(QString::number(nAmount * 0.01));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZUIDDControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZUIDDControlDialog::GetSelectedMints()
{
...
void ZUIDDControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
