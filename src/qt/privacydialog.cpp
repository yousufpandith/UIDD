                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zUIDD ought to be enough for anybody."
    ui->zUIDDpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzUIDDSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzUIDD->setEnabled(false);
        ui->pushButtonMintzUIDD->setToolTip(tr("zUIDD is currently disabled due to maintenance."));

        ui->pushButtonSpendzUIDD->setEnabled(false);
        ui->pushButtonSpendzUIDD->setToolTip(tr("zUIDD is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zUIDDpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzUIDD_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zUIDD is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zUIDD...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zUIDD in ") +
...
void PrivacyDialog::on_pushButtonSpendzUIDD_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zUIDD is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zUIDD
        sendzUIDD();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zUIDD
    sendzUIDD();
}

void PrivacyDialog::on_pushButtonZUIDDControl_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;

    ZUIDDControlDialog* zUIDDControl = new ZUIDDControlDialog(this);
    zUIDDControl->setModel(walletModel);
    zUIDDControl->exec();
}

void PrivacyDialog::setZUIDDControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzUIDDSelected_int->setText(QString::number(nAmount * 0.01));
    ui->labelQuantitySelected_int->setText(QString::number(nQuantity));
}

void PrivacyDialog::sendzUIDD()
{
    QSettings settings;
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Uidd Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zUIDDpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not able to be divided by 0.05 and change as zUIDD is requested
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zUIDD</b>";
    ...
    // use mints from zUIDD selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZUIDDControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZUIDDControlDialog::GetSelectedMints();
    }
    ...
    if (walletModel && walletModel->getAddressTableModel()) {
        // If zUIDD was spent successfully update the addressbook with the label
    ...
    // Clear zuidd selector in case it was used
    ZUIDDControlDialog::listSelectedMints.clear();
    ui->labelzUIDDSelected_int->setText(QString("0"));
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zUIDD Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " UIDD, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zUIDD Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zUIDD </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number((double)zerocoinBalance/COIN) + QString(" zUIDD "));
    ui->labelzAvailableAmount_2->setText(QString::number((double)matureZerocoinBalance/COIN) + QString(" zUIDD "));
    ui->labelzUIDDAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
    ...
    ui->labelZsupplyAmount->setText(QString::number(chainActive.Tip()->GetZerocoinSupply() * 0.00000001) + QString(" <b>zUIDD </b> "));
    for (auto denom : libzerocoin::zerocoinDenomList) {
        ...
        QString strSupply = QString::number(nSupply) + " x " + QString::number(denom * 0.01) + " = <b>" +
                            QString::number(nSupply*denom * 0.01) + " zUIDD </b> ";
        switch (denom) {
            case libzerocoin::CoinDenomination::ZQ_FIVECENTS:
...
{
    ui->labelzUIDDSyncStatus->setVisible(fShow);
}
