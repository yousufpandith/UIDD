...
static std::string ValueToString(CAmount nValue, bool AllowNegative = false)
{
    ...
    QString Str = BitcoinUnits::formatWithUnit(BitcoinUnits::UIDD, nValue);
...
static std::string ScriptToString(const CScript& Script, bool Long = false, bool Highlight = false)
{
    ...
    if (ExtractDestination(Script, Dest) && Address.Set(Dest)) {
        ...
        return Long ? "<pre>" + FormatScript(Script) + "</pre>" : _("zUIDD");
}
...
void BlockExplorer::showEvent(QShowEvent*)
{
    if (m_NeverShown) {
        ...
        if (!GetBoolArg("-txindex", true)) {
            QString Warning = tr("Not all transactions will be shown. To view all transactions you need to set txindex=1 in the configuration file (uidd.conf).");
            QMessageBox::warning(this, "Uidd Core Blockchain Explorer", Warning, QMessageBox::Ok);
        }
    }
}
