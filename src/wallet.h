...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT1000IFMN = 3,
    ONLY_NONDENOMINATED_NOT1000IFMN = 4, // ONLY_NONDENOMINATED and not 1000 UIDD at the same time
    ONLY_1000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zUIDD send
enum ZerocoinSpendStatus {
    ZUIDD_SPEND_OKAY = 0,                            // No error
    ZUIDD_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZUIDD_WALLET_LOCKED = 2,                         // Wallet was locked
    ZUIDD_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZUIDD_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZUIDD_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZUIDD_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZUIDD_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZUIDD_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZUIDD_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZUIDD_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZUIDD_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZUIDD_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZUIDD_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZUIDD_SPENT_USED_ZUIDD = 14,                      // Coin has already been spend
    ZUIDD_TX_TOO_LARGE = 15                          // The transaction is larger than the max tx size
};
...
public:
    ...
    */
    ...
    void setZUIDDAutoBackups(bool fEnabled)
    {
    ...
    /// Get 1000 UIDD output and keys which can be used for the Masternode
